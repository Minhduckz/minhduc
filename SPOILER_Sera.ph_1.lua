...
local window = library:new({textsize = 13.5,font = Enum.Font.RobotoMono,name = "Seraph // Android",color = Color3.fromRGB(244,255,25)})

local Main_1_Page = window:page({name = "Main"})
local Main_2_Page = window:page({name = "Main 2"})

local Main_1_left = Main_1_Page:section({name = "Farm",side = "left",size = 1000})
local Main_1_right = Main_1_Page:section({name = "Settings",side = "right",size = 250})
local Select_Farm_Mode = Main_1_left:dropdown({name = "Select Farm Type",def = "Level Farm",max = 4,options = {"Level Farm", "Ectoplasm", "Bone Farm", "Katakuri Farm"},callback = function(vu)
   getgenv().SeraphConfig.Farm.FarmType = vu
end})
local Start_farm = Main_1_left:toggle({name="Start Farm", def=false,callback=function(vu)
   getgenv().SeraphConfig.Farm.StartFarm = vu
end}) 
local Fast_atk = Main_1_right:toggle({name="Fast Attack", def=true,callback=function(vu)
  getgenv().SeraphConfig.Settings.FastAttack = vu 
  end})
local FastAttack_Delay = Main_1_right:dropdown({name="Fast Attack Delay", def = 0.15, max=6, options={1,0.75,0.5,0.35,0.15,0.1},callback=function(vu) 
 getgenv().SeraphConfig.Settings.FastAttackDelay = vu 
 end
 })
local Auto_Item_page = Main_1_Page:section({name="⏳ // Auto Items // ⏳", side="left", size=1000})  
local Auto_Buy_Rare_Item = Auto_Item_page:toggle({name="Auto Buy Uncommon Items", def = false, callback=function(vu) 
  getgenv().SeraphConfig.BuyItem.Uncommon = true 
  end})
